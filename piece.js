...
const getPawnAllowedMoves = (pawn) => {
	...
	// One step forward
	const oneStep = position + mathSign * 10;
	allowedMoves.push(oneStep);

	// Two steps forward from starting position
	if ((position >= 21 && position <= 28 && pawn.color === 'white') || 
		(position >= 71 && position <= 78 && pawn.color === 'black')) {
		allowedMoves.push(position + mathSign * 20);
	}

	// Attack moves diagonally
